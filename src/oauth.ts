...
const fail = () =>
  new Response(
    "GitHub sign-in could not be completed. Return to /login or /settings and try again.",
    { status: 400 },
  );
...
routes.get("/auth/github/callback", async (c) => {
  if (!githubEnabled(c.env)) return fail();
  const browser = getCookie(c, "rcve_oauth"),
    state = c.req.query("state"),
    code = c.req.query("code");
  if (!browser || !state || state.length > 128) return fail();
  ...
  if (!row || !code || code.length > 512 || c.req.query("error")) return fail();
  const current = await currentAccount(c);
  if ((row.account_id || null) !== (current?.id || null)) return fail();
  try {
    ...
    if (!res.ok || typeof credentials.access_token !== "string" || credentials.error) return fail();
